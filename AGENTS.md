...
- `record Name field Type ... end` defines a nominal record type. `Name:alloc` has stack effect `-- Name*`, `Name:sizeof` has stack effect `-- int`, and `@Name.field`/`!Name.field` receive `Name*` values.
- `@.field` and `!.field` infer the nominal record type from the top static `Name*` stack value after macro expansion. Exact macros with those spellings take precedence; cover direct, macro-expanded, and imported-alias access when changing this behavior.
- Record fields occupy one Cell in declaration order. Fields that refer to allocated records use `Name*`, and explicit `ptr`/record-pointer casts cross the nominal boundary.
